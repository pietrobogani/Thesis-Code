...
# y(t)  ~ sampled from original data
# NFCI(t) ~ sampled from original data + gaussian noise
...
mean <- 0 
sd <- 2
#skewness <- alpha  #skewness of skew-t. 
...
#GENERATE DATA
y <- sample(data$A191RL1Q225SBEA, size = 1004, replace = TRUE)
#y <-  rnorm(1004, mean = m1, sd = var1) # simulate GDP growth quarter by quarter

NFCI <- sample(data$NFCI, size = 1004, replace = TRUE)
NFCI <- NFCI + rnorm(1004,0,0.2) #To add some variability

Yh <- filter(y, rep(1/h, h), sides=1) * tau # moving average of the h past values

# if (runif(1) < 0.66) { #I make it such 66% of the time the noise is left-skewed
#   alpha <- runif(1,0,2)
# } else {
#   alpha <- runif(1,-2,0)
# }
# skew_prop <- c(mean=mean, sd=sd, gamma1=alpha, gamma2 = gamma2) #alpha >0 => left-skewed
...
for (jq in 1:length(QQ)) {
  ...
  #FULL CONDITIONAL
  QRfull <- rq(Yh[(h+1):length(Yh)] ~ Z[1:(length(Yh) - h),-1], tau=QQ[jq])
  Quant_IS[(h+1):length(Yh), jq] <- as.vector(Z[1:(length(Yh) - h),] %*% coef(QRfull)) 
  
  #CONDITIONAL, GDP ONLY
  QR <- rq(Yh[(h+1):length(Yh)] ~ ZGDPonly[1:(length(Yh) - h),-1], tau=QQ[jq])
  QuantGDPonly_IS[(h+1):length(Yh), jq] <- as.vector(ZGDPonly[1:(length(Yh) - h),] %*% coef(QR)) 
...
for (jt in jtFirstOOS:(length(Yh)-h)) {
    for (jq in 1:length(QQ)) {  
      ...
      #FULL CONDITIONAL
      QRfull <- rq(Yh[(h+1):jt] ~ Z[1:(jt - h),-1], tau=QQ[jq])
      Quant_OOS[jt + h, jq] <- Z[jt, ] %*% coef(QRfull)
      
      
      #CONDITIONAL, GDP ONLY
      QR <- rq(Yh[(h+1):jt] ~ ZGDPonly[1:(jt - h),-1], tau=QQ[jq])
      QuantGDPonly_OOS[jt + h, jq] <- ZGDPonly[jt, ] %*% coef(QR)
...
ZGDPonly2 <- ZGDPonly[(length/2+1):(length(Yh) - h),]
...
for (jq in 1:length(QQ)) {
  ...
  Q_high[(h+1):(h+length(Yh2)), jq] <- as.vector(ZGDPonly2 %*% coef(QR)) 
  ...
  CQuantGDPonly_IS[(h+1):length(Yh) ,jq] <- as.vector(ZGDPonly[1:(length(Yh) - h),] %*% coef(QR)) + quantile_E
...
for (jt in jtFirstOOS:(length(Yh)-h)) {
    ...
    for (jq in 1:length(QQ)) {  
      ...
      CQuantGDPonly_OOS[jt+h,jq] <- ZGDPonly[jt,] %*% coef(QR) + quantile_E
...
for (jt in 1:(length(Yh) - h)) {
  ...
  YhRealized <- Yh[jt + h]
  qqTarg <- QuantGDPonly_IS[jt + h, ]
  PitSTGDPonly_IS[jt + h] <- cumulative_prob(YhRealized, QQ, qqTarg)
}
...
#OUT OF SAMPLE, CONDITIONAL GDP ONLY
  ...
  for (jt in jtFirstOOS:(length(Yh)-h)){
      ...
      YhRealized <- Yh[jt + h]
      qqTarg <- QuantGDPonly_OOS[jt + h, ]
      PitSTGDPonly_OOS[jt + h] <- cumulative_prob(YhRealized, QQ, qqTarg)
...
for (jt in 1:(length(Yh) - h)) {
  ...
  YhRealized <- Yh[jt + h]
  qqTarg <- CQuantGDPonly_IS[jt + h, ]
  CPitSTGDPonly_IS[jt + h] <- cumulative_prob(YhRealized, QQ, qqTarg)
}
...
#OUT OF SAMPLE, CONDITIONAL GDP ONLY
  ...
  for (jt in jtFirstOOS:(length(Yh)-h)){
      ...
      YhRealized <- Yh[jt + h]
      qqTarg <- CQuantGDPonly_OOS[jt + h, ]
      CPitSTGDPonly_OOS[jt + h] <- cumulative_prob(YhRealized, QQ, qqTarg)
