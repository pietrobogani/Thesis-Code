...
legend("topleft", 
       legend = c("LeftEntropy_OOSCO", "LeftEntropy_OOS"), 
       col = c('#0000FF', '#008000'),
       lty = c(1, 1),  
       lwd = c(2, 2))  
...
legend("topleft", 
       legend = c("LeftEntropy_ISCO", "LeftEntropy_IS"), 
       col = c('#0000FF', '#008000'),
       lty = c(1, 1),  
       lwd = c(2, 2))  
```
...
results <- compute_results(coverage, n, QQC)
...
plot1 <- ggplot(df, aes(x = Quantile, y = EmpiricalCoverage, color = Group)) +
  ...
  scale_color_manual(values = c("NFCI CQR QR" = pitc_colors, "NFCI Adrian et al." = pit_colors)) + # Color scale
...
print(plot1)
ggsave(filename = paste0("fulloppositeNFCIvsOriginal", h, ".pdf"), plot = plot1, width = 7, height = 5)
