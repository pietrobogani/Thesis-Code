...
\puttitle{
	title=Conformal Quantile Regression, % Title of the thesis
	name=Pietro Bogani, % Author Name and Surname
	course=Mathematical Engineering - Ingegneria Matematica, % Study Programme (in Italian)
	ID  = 10622041,  % Student ID number (numero di matricola)
	advisor= Prof. Simone Vantini, % Supervisor name
	coadvisor={Matteo Fontana, Luca Neri}, % Co-Supervisor name, remove this line if there is none
...
% ABSTRACT IN ENGLISH
\chapter*{Abstract} 
Conformal Prediction is a framework for creating rigorous prediction sets/intervals.
Importantly, the results are achieved in a distributional-free scenario: no underlying assumptions on the distribution of the data is needed, allowing maximum flexibility in a variety of scenarios.
Furthermore, it is model-agnostic, as it can be implemented on top of any machine learning model, it comes with mathematically proven guarantees on the coverage of the sets and in also relatively easy to be understood and implemented. 
...
\\
\\
\textbf{Keywords:} Conformal Prediction, Growth at Risk, Quantile Regression,  % Keywords
...
add what the single chapter in the thesis is about
