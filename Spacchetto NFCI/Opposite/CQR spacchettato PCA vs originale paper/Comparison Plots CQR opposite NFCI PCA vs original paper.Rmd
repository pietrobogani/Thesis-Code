...
```{r}
TimeOOS <- Time[(1+sum(is.na(YQ_OOS[,1]))):length(Time)]
plot(TimeOOS, YQ_OOS[, jq05][!is.na(YQ_OOS[, jq05])], type = 'l', col = 'blue', xlab = 'Time', ylab = 'Quantiles', xlim = range(Time), ylim = c(-20,20))
lines(TimeOOS, YQ_OOSC[, jq05][!is.na(YQ_OOSC[, jq05])], type = 'l', col = 'blue', lty = 2)
```
...
coverage <- calculate_percent_below(PitST_OOS,QQ)/100
...
plot1 <- ggplot(df, aes(x = Quantile, y = EmpiricalCoverage, color = Group)) +
  ...
  scale_color_manual(values = c("CQR QR NFCI's Components PCA" = pitc_colors, "NFCI Adrian et al." = pit_colors)) + # Color scale
