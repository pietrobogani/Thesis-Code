...
filename <- paste("ResOOSCQRFspacchetto_H", h, ".RData", sep="")
...
df1 <- data.frame(
  ...
  Group = "CQR QRF NFCI's components"
)

df2 <- data.frame(
  Quantile = data1$tau,
  EmpiricalCoverage = data1$QR,
  Group = "QRF NFCI's components"
)
...
plot1 <- ggplot(df, aes(x = Quantile, y = EmpiricalCoverage, color = Group)) +
  ...
  scale_color_manual(values = c("CQR QRF NFCI's components" = pitc_colors, "QRF NFCI's components" = pit_colors)) + # Color scale
