...
df2 <- data.frame(
  ...
  Group = "QR NFCI's components PCA"
)
...
plot1 <- ggplot(df, aes(x = Quantile, y = EmpiricalCoverage, color = Group)) +
  ...
  scale_color_manual(values = c("CQR QR NFCI's components PCA" = pitc_colors, "QR NFCI's components PCA" = pit_colors)) + # Color scale
...
print(plot1)
ggsave(filename = paste0("fulloppositeQRvsCQRspacchettatoPCA", h, ".pdf"), plot = plot1, width = 7, height = 5)
