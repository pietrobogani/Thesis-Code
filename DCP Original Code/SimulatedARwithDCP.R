...
run_simulation <- function(){
  ...
  n <- 204
  jtFirstOOS <- 200 #First index for out-of-sample computations
  source("temp1.R")
  source("full.R")
  funs=rq.funs()
...
for (i in 2:n_simul) {
  cat("Simulazione numero", i, "è iniziata\n")
  set.seed(seeds[i])
  ret <- run_simulation()
  PitSTAR1_OOStot <- c(PitSTAR1_OOStot, na.omit(ret[[1]]))
  DPitSTAR1_OOStot <- c(DPitSTAR1_OOStot, na.omit(ret[[2]]))
}
...
# Count the number of times SuccessRate is above the Quantile when IsWithinCI is FALSE
count_above <- sum(resultsDPitST$SuccessRate > resultsDPitST$Quantile & !resultsDPitST$IsWithinCI)

# Count the number of times SuccessRate is below the Quantile when IsWithinCI is FALSE
count_below <- sum(resultsDPitST$SuccessRate < resultsDPitST$Quantile & !resultsDPitST$IsWithinCI)
