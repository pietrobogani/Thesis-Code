...
file_path <- "ALLINONE_iid_Results.xlsx"
...
run_simulation <- function(n,ratio_p_n){
  ...
  indices <- c((n+1):(n+n2))
  data <- data.frame(I = 1, Y = Y[-indices])
  for (j in 1:num_exog_vars) {
    data[paste0("X", j)] <- exog_vars[-indices, j]
  }
  
  
  data_test <- data.frame(I = 1, Y = Y[indices])
  for (j in 1:num_exog_vars) {
...
#--------------- QUANTILE REGRESSION 
  ...
  Quant_OOS <- matrix(NA, n2, length(QQ))
  #qrf_model <- quantregForest(x = as.matrix(data[,-c(1,2)]), y = as.matrix(data[,2]))
  

    for (jq in 1:length(QQ)) {  
       
       QR <- rq(make_formula(0,num_exog_vars), data = data, tau = QQ[jq])     
       Quant_OOS[,jq] <- as.matrix(data_test[,-2]) %*% coef(QR)
      
       #QuantAR_OOS[,jq] <- predict(qrf_model, newdata = as.matrix(data_test[,-c(1,2)]), what = QQ[jq])
...
writeData(wb, sheet = sheet, x = paste("n1 = ", n,", p/n = ", p/n, ", QR" ), startCol = 1, startRow = count, colNames = FALSE)
writeData(wb, sheet = sheet, x = paste("n1 = ", n,", p/n = ", p/n,", CQR" ), startCol = 1, startRow = count+1, colNames = FALSE)
