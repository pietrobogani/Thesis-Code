...
file_path <- "ALLINONE_Exogenous_Covariates_Misspecified_Results.xlsx"
...
run_simulation <- function(n,ratio_p_n,alpha){
  ...
  num_selected_vars <- ceiling(alpha * num_exog_vars)  # alpha% of the exogenous variables
  selected_vars_indices <- sample(1:num_exog_vars, num_selected_vars)
  num_exog_vars <- num_selected_vars
  ...
  for (j in 1:num_selected_vars) {
    data[paste0("X", j)] <- exog_vars[-c(1:3, indices), selected_vars_indices[j]]
  }
  
  
  data_test <- data.frame(I = 1, Y = Y_ar2[indices], Y_lag1 = Y_lag1_ar2[indices], Y_lag2 = Y_lag2_ar2[indices], Y_lag3 = Y_lag3_ar2[indices])
  for (j in 1:num_selected_vars) {
    data_test[paste0("X", j)] <- exog_vars[indices, selected_vars_indices[j]]
  }
