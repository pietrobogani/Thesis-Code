...
file_path <- "ALLINONE_Cauchy_Results_Forest_Opposite.xlsx"
#file_path <- "ALLINONE_Cauchy_Results_Opposite.xlsx"
...
#--------------- QUANTILE REGRESSION 
  ...
  QuantAR3_OOS <- matrix(0, n2, length(QQ))
      ...
      #    
      # #     QR1 <- rq(Y ~ Y_lag1, data = data, tau=(1-QQ[jq]))
      # #     QuantAR1_OOS[,jq] <- as.matrix(data_test[,-c(2,4,5)]) %*% coef(QR1)
      # #     
             # QR2 <- rq(Y ~ Y_lag1 + Y_lag2 , data = data, tau=(1-QQ[jq]))
             # QuantAR2_OOS[,jq] <- as.matrix(data_test[,-c(2,5)]) %*% coef(QR2)
      #      
      # #     QR3 <- rq(Y ~ Y_lag1 + Y_lag2 + Y_lag3, data = data, tau=(1-QQ[jq]))
...
#--------------- QUANTILE RANDOM FOREST 
  ...
  # qrf_model3 <- quantregForest(x = as.matrix(data[, c(3,4,5)]), y = data[,2])
    ...
    for (jq in 1:length(QQ)) {  
     ...
     QuantAR2_OOS[,jq] <- predict(qrf_model2, newdata = as.matrix(data_test[,c(3,4)]), what = (1-QQ[jq]))      
      # QuantAR3_OOS[,jq] <- predict(qrf_model3, newdata = as.matrix(data_test[,c(3,4,5)]), what = (1-QQ[jq]))
...
#--------------- CONFORMALIZED QUANTILE REGRESSION
  ...
  data_2 <- data[(test_length + 1) : full_length,] #calibration
   ...
   # 
   # CQuantAR1_OOS <- qCQR_opposite(formula,x0,y0,x1,y1,x_test,QQ)
   ...
   #AR2
      ...
      #  
      #  CQuantAR2_OOS <- qCQR_opposite(formula,x0,y0,x1,y1,x_test,QQ)
    # 
   # #AR2
   # x0 <- data_1[,-2]
   # y0 <- data_1[,2]
   ...
   # 
   # CQuantAR3_OOS <- qCQR_opposite(formula,x0,y0,x1,y1,x_test,QQ)
...
writeData(wb, sheet = sheet, x = paste("n1 = ", n-3, ", QR AR(1)" ), startCol = 1, startRow = count, colNames = FALSE)
writeData(wb, sheet = sheet, x = paste("n1 = ", n-3, ", CQR AR(1)" ), startCol = 1, startRow = count+1, colNames = FALSE)
writeData(wb, sheet = sheet, x = paste("n1 = ", n-3, ", QR AR(2)" ), startCol = 1, startRow = count+2, colNames = FALSE)
writeData(wb, sheet = sheet, x = paste("n1 = ", n-3, ", CQR AR(2)" ), startCol = 1, startRow = count+3, colNames = FALSE)
writeData(wb, sheet = sheet, x = paste("n1 = ", n-3, ", QR AR(3)" ), startCol = 1, startRow = count+4, colNames = FALSE)
writeData(wb, sheet = sheet, x = paste("n1 = ", n-3, ", CQR AR(3)" ), startCol = 1, startRow = count+5, colNames = FALSE)
...
p <- ggplot(df, aes(x = Quantile, y = EmpiricalCoverage, color = Group)) +
  ...
  labs(title = paste("n1 =", n - 3), x = "Quantile Levels", y = "Empirical Coverage") + # Add labels and title
  theme_minimal() +
  theme(
...
ggsave(filename = paste0("Cauchy_calibration_n", n - 3, "_AR(2)_RForest.pdf"), plot = p, width = 7, height = 5)
