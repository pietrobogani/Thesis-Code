...
calculate_crps_from_quantiles <- function(observed, quantile_values, quantile_levels) {
  
  # Ensure quantile levels and values are sorted
  quantile_values <- sort(quantile_values)
  
  y_grid <- seq(min(quantile_values), max(quantile_values), length.out = 200)
  
  grid_levels <- numeric(length(y_grid))
  
  for (i in 1:length(y_grid)) {
    # Find the highest value in quantile_values that is <= current grid point
    grid_levels[i] <- max(quantile_levels[quantile_values <= y_grid[i]])
  }
  
  n_obs <- length(observed)
  crps_values <- rep(NA,n_obs)
  weights <- c(diff(y_grid),NA) # * (2*quantile_levels-1)^2 # To approximate an integral with a sum, I will use "rectangles", this is their base
                                          # The last element gets lost! There's no next one to "build a rectangle with"
  
  for(i in 1:n_obs){
    obs <- observed[i]
    heaviside_values <- as.numeric(y_grid >= obs)  # Heaviside step function values
    crps_values[i] <- sum((grid_levels - heaviside_values)^2 * weights, na.rm = TRUE)
  }
...
write_in_excel <- function(wb, sheet, x, startCol, startRow, colNames = FALSE) {
