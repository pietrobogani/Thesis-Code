...
file_path <- "ALLINONE_Exogenous_Results_Opposite_TEST_BUTTOVIA.xlsx"
#file_path <- "ALLINONE_Exogenous_Results_Opposite_TEST.xlsx"
...
run_simulation <- function(n,ratio_p_n){
  ...
  QuantAR3_OOS <- matrix(0, n2, length(QQ))
     ...
     for (jq in 1:length(QQ)) {  
      ...
      # QR1 <- rq(make_formula(1, num_exog_vars), data = data, tau=(1-QQ[jq]))
      # QuantAR1_OOS[,jq] <- as.matrix(data_test[,-c(2,4,5)]) %*% coef(QR1)
        
       QR2 <- rq(make_formula(2, num_exog_vars), data = data, tau=(1-QQ[jq]))
       QuantAR2_OOS[,jq] <- as.matrix(data_test[,-c(2,5)]) %*% coef(QR2)
        
       # QR3 <- rq(make_formula(3, num_exog_vars), data = data, tau=(1-QQ[jq]))
